...
let ThfcPdpStub = {
  belongsTo: sinon.spy() // Argumentos para la clase
...
test.beforeEach(async () => {
  ...
  TypeStub.update.withArgs(typeFixtures.single, {
    where: {
      tipo: typeFixtures.single.tipo
    }
  }).returns(Promise.resolve(typeFixtures.single)); // Type#Model create Stub
  ...
  TypeStub.findAll.withArgs().returns(Promise.resolve(typeFixtures.findAll)); // Type#Model findByClassId Stub

  TypeStub.findByClassId = sandbox.stub();
  TypeStub.findByClassId.withArgs(classId).returns(Promise.resolve(typeFixtures.findByClassId(classId)));
...
test.serial('Type#createOrUpdate - exist', async t => {
  ...
  t.true(_rec13._expr(_rec13._capt(_rec13._capt(_rec13._capt(TypeStub, "arguments/0/object/object").findOne, "arguments/0/object").calledTwice, "arguments/0"), {
    content: "t.true(TypeStub.findOne.calledTwice, 'findOne should be called twice')",
    filepath: "tests/type-test.js",
    line: 199,
    async: true,
...
test.serial('Type#findByClassId', async t => {
  let types = await db.Type.findByClassId(classId);
  t.deepEqual(types, typeFixtures.findByClassId(classId), 'types should be the same');
});
//# sourceMappingURL=/var/www/html/planner-db/node_modules/.cache/ava/832e1d2fbb430dd79540e877ee18d1c4.js.map